...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (new URLSearchParams(window.location.search).get("sent") === "1") {
    ...
    if (successBox) {
      ...
      // Poczekaj chwilę aż strona się w pełni wyrenderuje, potem przewiń do komunikatu
      setTimeout(() => {
        successBox.scrollIntoView({ behavior: "smooth", block: "center" });
      }, 150);
    }

    // Czyścimy parametr z adresu, żeby odświeżenie/powrót nie pokazywało
    // znów tego samego komunikatu przy zwykłym wejściu na stronę
